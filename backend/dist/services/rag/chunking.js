...
exports.documentChunkingService = exports.DocumentChunkingService = void 0;
...
class DocumentChunkingService {
     ...
     */
    async chunkDocument(documentId, content, title, options) {
        ...
        try {
            ...
            const chunks = this.createChunks(documentId, cleanedContent, title, language, chunkingOptions);
            logger_1.logger.info({
    ...
    detectLanguage(content) {
        ...
        const frenchScore = frenchWords.reduce((score, word) => score + (sample.includes(` ${word} `) ? 1 : 0), 0);
        const englishScore = englishWords.reduce((score, word) => score + (sample.includes(` ${word} `) ? 1 : 0), 0);
        return frenchScore > englishScore ? 'fr' : 'en';
    }
    /**
     * Création des chunks avec préservation des structures
     */
    createChunks(documentId, content, title, language, options) {
        const chunks = [];
        ...
        for (const paragraph of paragraphs) {
            ...
            if (paragraphTokens > options.maxTokens) {
                ...
                if (currentChunk.trim().length > 0) {
                    chunks.push(this.createChunk(documentId, currentChunk.trim(), title, language, chunkIndex++, startPosition, startPosition + currentChunk.length, currentTokens));
     ...
     */
    createChunk(documentId, content, title, language, chunkIndex, startPosition, endPosition, tokenCount) {
        return {
     ...
     */
    getOverlapLength(text, overlapTokens) {
...
// Instance singleton
exports.documentChunkingService = new DocumentChunkingService();
